    ...
    def time_domain(self, parameters=None, times=None, **kwargs):
        ...
        if not (self._args == self._cache_key):
            ...
            if times is not None:
                ...
                times_wf = (
                    array_library.arange(len(hp.data.data)) * float(hp.deltaT)
                    + float(hp.epoch) + float(epoch)
                )
                spl_hp = CubicSpline(times_wf, hp.data.data, extrapolate=False)
                spl_hx = CubicSpline(times_wf, hx.data.data, extrapolate=False)
                hp_data = spl_hp(times.value)
                hx_data = spl_hx(times.value)
                hp_data[np.isnan(hp_data)] = 0
                hx_data[np.isnan(hx_data)] = 0
